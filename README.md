...
- Put your photos in **`/storage/Slideshow/photos/`** on the box (supports `.jpg`, `.jpeg`, `.png`, `.webp`, `.bmp`, `.gif`). The folder is created automatically the first time the slideshow runs if it doesn't already exist.
- Configure everything via each addon's **Settings** screen (the gear icon shown on its info panel in `Add-ons > My add-ons`):
  - **TV Slideshow** settings:
    ...
    - *Random order* — shuffle the photo order on each launch (default on).
...
- Each photo is scaled according to the *Photo fit* setting above, and always fills whatever resolution Kodi's GUI is currently running at (1080p, 4K, etc.), since the display area is read live from Kodi rather than hardcoded.
...
- Relaunching the addon re-scans the photos folder, re-shuffles (if enabled), and re-reads its settings, so adding/removing photos or changing settings doesn't require reinstalling anything — just relaunch (or wait for auto-start).
...
```bash
scp myphoto1.jpg myphoto2.jpg root@<box-ip>:/storage/Slideshow/photos/
```
No reinstall or settings change needed — just relaunch the slideshow (or wait for the auto-start service) to pick up new/removed photos.
