...
/usr/bin/git push
```
This regenerates `repo/addons-manifest.xml`, `repo/addons-manifest.xml.md5.txt`, and each addon's zip under `repo/<addon-id>/`, served via `raw.githubusercontent.com`. The GitHub repo must stay **public** for this to work, since Kodi can't authenticate.

Two CDN gotchas already worked around, so they don't need rediscovering:
- The manifest is named `addons-manifest.xml` rather than the conventional `addons.xml`, because `raw.githubusercontent.com` cached a 404 against the plain `addons.xml` path back when this repo was briefly private during setup, and that negative cache entry never expired on its own even long after the repo went public. A path with no prior caching history has no such baggage - if this ever needs renaming again, always pick something never previously requested rather than reusing a path that might have a stale entry.
- The checksum file is named `.md5.txt` rather than `.md5` because [jsdelivr](https://www.jsdelivr.com/) - tried before settling on `raw.githubusercontent.com`, and abandoned because its cache proved inconsistent across edge nodes for a moving `@main` branch reference, unpredictably serving different versions depending which edge a request happened to hit - blocks the plain `.md5` extension outright. Kodi only cares about the URL in the repository addon's `<checksum>` tag, not the filename, so this naming is purely cosmetic and was kept for consistency even after moving off jsdelivr.

Kodi periodically re-checks the checksum URL (or checks immediately if you open `Settings > Add-ons > My add-ons` and pull to refresh) and shows its normal Update button once the new version is live - verify directly with `curl` against the URLs in `repository.muggehslideshow/addon.xml` if in doubt; a fresh, never-before-requested path should reflect a new push immediately. The repository addon is itself included in this same build/publish step, so its own update mechanism can update itself too if its `addon.xml` ever changes.
